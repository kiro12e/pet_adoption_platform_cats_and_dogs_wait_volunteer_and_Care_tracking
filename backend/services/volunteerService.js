...
class VolunteerService{
    ...
    async signUpHandleRegister(conn){
      ...
      const existing = await repo.FindVolunteerByEmail ? await repo.FindVolunteerByEmail(volunteerEmail) : await repo.findVolunteerByEmail(conn, volunteerEmail);
      if(existing){ //If the email already exists in the database, registration is not allowed.
            await conn.rollback();
            conn.release();
